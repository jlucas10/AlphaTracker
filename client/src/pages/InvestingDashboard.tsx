...
export default function InvestingDashboard() {
    ...
    const [formData, setFormData] = useState({
        ...
        trade_type: 'LONG',
        setup: 'Breakout',
        account_id: '' // NEW: Track account link
    });

    // Fetch Trades and Accounts on load
    useEffect(() => {
        if (user) {
            fetchTrades();
            fetchAccounts();
    ...
    const fetchAccounts = async () => {
        try {
            ...
            if (data.length > 0) {
                setFormData(prev => ({ ...prev, account_id: data[0].account_id.toString() }));
            }
        } catch (error) {
    ...
    const processChartData = (tradeList: Trade[]) => {
        ...
        tradeList.forEach(trade => {
            ...
            allocation[trade.ticker] = (allocation[trade.ticker] || 0) + value;
        });
    ...
    const deleteTrade = async (id: number) => {
        if (!window.confirm("Delete this investment? Cost will be refunded to the linked account.")) return;
        try {
            ...
            <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1 bg-white p-6 rounded-xl border border-gray-200 shadow-sm h-fit">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div>
                            <label className="block text-xs text-gray-500 uppercase font-bold mb-1">Funding Account</label>
                            <select
                                name="account_id"
                            ...
                            >
                                ...
                                {accounts.map(acc => (
                                    <option key={acc.account_id} value={acc.account_id}>
                                        🏦 {acc.account_name} (${Number(acc.balance).toLocaleString()})
                                    </option>
                                ))}
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <div className="flex gap-2">
                                <input name="ticker" value={formData.ticker} onChange={handleChange} placeholder="NVDA" className={`${inputStyle} uppercase`} required />
                                <button type="button" onClick={fetchPrice} disabled={isLoadingPrice} className="bg-gray-100 hover:bg-gray-200 border border-gray-300 text-gray-900 px-4 rounded font-bold text-sm transition disabled:opacity-50">
                                    {isLoadingPrice ? "⏳" : "🔍"}
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <input name="entry_price" type="number" step="0.01" value={formData.entry_price} onChange={handleChange} placeholder="150.00" className={inputStyle} required />
                            </div>
                            <div>
                                <label className="block text-xs text-gray-500 uppercase font-bold mb-1">Shares</label>
                                <input name="shares" type="number" step="0.01" value={formData.shares} onChange={handleChange} placeholder="10" className={inputStyle} required />
                            </div>
                        </div>
                        ...
                        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm flex flex-col justify-center">
                            ...
                            <div className="mt-4 pt-4 border-t border-gray-100">
                                ...
                                <p className="text-xl font-mono text-gray-900 font-medium">
                                    {trades.length > 0 ? trades[trades.length - 1].ticker : "--"}
                                </p>
                            </div>
                        </div>
                    ...
                    <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-left text-sm">
                                ...
                                {trades.map((trade) => {
                                    const linkedAcc = accounts.find(a => a.account_id === trade.account_id);
                                    return (
                                        <tr key={trade.trade_id} className="hover:bg-gray-50 transition">
                                            <td className="p-4 font-bold text-gray-900">{trade.ticker}</td>
                                            <td className="p-4 text-xs text-gray-500">{linkedAcc?.account_name || 'N/A'}</td>
                                            <td className="p-4">
                                            ...
                                            <td className="p-4 font-mono text-gray-600">${trade.entry_price} x {trade.shares}</td>
                                            <td className="p-4 text-right">
                                                <button onClick={() => deleteTrade(trade.trade_id)} className="text-gray-400 hover:text-red-500 transition p-1 hover:bg-red-50 rounded">🗑️</button>
                                            </td>
                                        </tr>
                                    );
                                })}
